...
const TeacherRegistrationForm: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-600 via-green-700 to-emerald-800 py-8 sm:py-12 px-3 sm:px-6 lg:px-8 relative overflow-hidden">
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-0 w-72 h-72 sm:w-96 sm:h-96 bg-teal-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
        <div className="absolute top-0 right-0 w-72 h-72 sm:w-96 sm:h-96 bg-green-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000"></div>
        <div className="absolute bottom-0 left-1/2 w-72 h-72 sm:w-96 sm:h-96 bg-emerald-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
      </div>

      <div className="max-w-4xl mx-auto relative z-10">
        <div className="text-center mb-6 sm:mb-8">
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-2 drop-shadow-lg">Teacher Registration</h2>
          <p className="text-sm sm:text-base text-teal-100">Experienced educators joining our platform</p>
        </div>

        <div className="bg-white/95 backdrop-blur-lg dark:bg-slate-800/95 rounded-2xl shadow-2xl p-4 sm:p-6 lg:p-8 border border-white/20">
          {/* Tabs */}
          <div className="mb-6 sm:mb-8 -mx-4 sm:mx-0">
            <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700 overflow-x-auto px-4 sm:px-0 scrollbar-thin">
              {tabs.map((tab, index) => (
                <button
                  key={index}
                  type="button"
                  onClick={() => setActiveTab(index)}
                  className={`flex-shrink-0 px-3 py-2 font-medium text-xs transition-colors whitespace-nowrap ${
                    activeTab === index
